...
namespace Catch {
    ...
    Stream createStream(std::string const
    & streamName );
...
namespace Catch {
    ...
    void writeToDebugConsole(std::string const
    & text );
}
...
namespace Catch {
    namespace Matchers {
        namespace Impl {
            ...
            namespace StdString {
                ...
                inline std::string makeString(std::string const
                & str ) {
                return
                str;
            }
    ...
    inline Impl::StdString::Equals Equals(std::string const
    & str ) {
    return
    Impl::StdString::Equals( str );
}
...
& substr ) {
return
Impl::StdString::Contains( substr );
}
...
& substr ) {
return
Impl::StdString::StartsWith( substr );
}
...
& substr ) {
return
Impl::StdString::EndsWith( substr );
}
...
namespace Catch {
    ...
    inline TestSpec parseTestSpec(std::string const
    & arg ) {
    return

    TestSpecParser( ITagAliasRegistry::get())
...
STITCH_CLARA_OPEN_NAMESPACE
    ...
    const unsigned int consoleWidth = CLARA_CONFIG_CONSOLE_WIDTH;
            ...
            inline bool startsWith(std::string const
            & str,
            std::string const &prefix
            ) {
            return str.
        ...
        inline void convertInto(std::string const
        & _source,
        std::string &_dest
        ) {
        _dest = _source;
...
namespace Catch {
    ...
    inline void fatal(std::string const
    & message,
    int exitCode
    ) {
...
namespace Catch {
    ...
    AssertionInfo::AssertionInfo(std::string const
    & _macroName,
    SourceLineInfo const &_lineInfo,
            std::string const
    & _capturedExpression,
...
namespace Catch {
    ...
    return
    TestCaseInfo::ShouldFail;
    else if( tag == "!mayfail" )
    return
    TestCaseInfo::MayFail;
    else
    return
    TestCaseInfo::None;
}
inline bool isReservedTag(std::string const
& tag ) {
return
parseSpecialTag( tag )
==
TestCaseInfo::None &&tag
.
...
inline void enforceNotReservedTag(std::string const
& tag,
SourceLineInfo const &_lineInfo
) {
if(
...
return
TestCase( _testCase, info
);
}

TestCaseInfo::TestCaseInfo(std::string const
& _name,
std::string const &_className,
        std::string const
& _description,
...
TestCaseInfo::TestCaseInfo(TestCaseInfo const
& other )
:
name( other
.name ),
...
TestCase::TestCase(ITestCase * testCase, TestCaseInfo const
& info ) :
...
TestCase TestCase::withName(std::string const
& _newName ) const {
TestCase other(*this);
other.
name = _newName;
...
bool TestCase::operator==(TestCase const
& other ) const {
return  test.
...
namespace Catch {
    ...
    MessageInfo::MessageInfo(std::string const
    & _macroName,
    SourceLineInfo const &_lineInfo,
            ResultWas::OfType
    _type )
...
& testInfo ) {
m_legacyReporter->
StartTestCase( testInfo );
}
void LegacyReporterAdapter::sectionStarting(SectionInfo const
& sectionInfo ) {
m_legacyReporter->
StartSection( sectionInfo
.name, sectionInfo.description );
}
...
& assertionStats ) {
if( assertionStats.assertionResult.

getResultType()

!= ResultWas::Ok ) {
for(
std::vector<MessageInfo>::const_iterator it = assertionStats.infoMessages.begin(), itEnd = assertionStats.infoMessages.end();
it !=
itEnd;
++it ) {
...
m_legacyReporter->
NoAssertionsInSection( sectionStats
.sectionInfo.name );
m_legacyReporter->
EndSection( sectionStats
.sectionInfo.name, sectionStats.assertions );
}
...
m_legacyReporter->
EndGroup( testGroupStats
.groupInfo.name, testGroupStats.totals );
}
void LegacyReporterAdapter::testRunEnded(TestRunStats const
& testRunStats ) {
m_legacyReporter->
EndTesting( testRunStats
.totals );
}
...
return start != std::string::npos ? str.
substr( start,
1+end-start ) : "";
}
...
SourceLineInfo::SourceLineInfo(SourceLineInfo const
& other )
:
file( other
.file ),
...
bool SourceLineInfo::operator==(SourceLineInfo const
& other ) const {
return line == other.
line &&file
== other.
...
namespace Catch {
    ...
    ResultBuilder &ResultBuilder::setLhs(std::string const
    & lhs ) {
    m_exprComponents.
    lhs = lhs;
    return *this;
}
ResultBuilder &ResultBuilder::setRhs(std::string const
& rhs ) {
m_exprComponents.
rhs = rhs;
return *this;
}
ResultBuilder &ResultBuilder::setOp(std::string const
& op ) {
m_exprComponents.
op = op;
return *this;
...
namespace Catch {
    ...
    Option<TagAlias> TagAliasRegistry::find(std::string const
    & alias ) const {
...
for(
std::map<std::string, TagAlias>::const_iterator it = m_registry.begin(), itEnd = m_registry.end();
it !=
itEnd;
++it ) {
